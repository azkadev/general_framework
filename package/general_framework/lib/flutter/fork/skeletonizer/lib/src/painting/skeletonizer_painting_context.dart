...
class SkeletonizerGeneralFrameworkPaintingContext extends PaintingContext {
  ...
  @override
  ui.Canvas get canvas => isZone ? super.canvas : SkeletonizerGeneralFrameworkCanvas(super.canvas, this);
...
class SkeletonizerGeneralFrameworkCanvas implements Canvas {
  ...
  void drawParagraph(ui.Paragraph paragraph, ui.Offset offset) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      final shouldJustify = _config.justifyMultiLineText && isNotCentered && (lines.length > 1 && i < (lines.length - 1));
      ...
      final borderRadius = _config.textBorderRadius.usesHeightFactor ? BorderRadius.circular((fontSize + fontDescent) * _config.textBorderRadius.heightPercentage!) : _config.textBorderRadius.borderRadius?.resolve(TextDirection.ltr);
