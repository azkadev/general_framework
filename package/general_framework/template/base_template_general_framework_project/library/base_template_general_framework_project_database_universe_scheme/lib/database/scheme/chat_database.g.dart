...
extension GetChatDatabaseCollection on DatabaseUniverse {
  DatabaseUniverseCollection<int, ChatDatabase> get chatDatabases =>
      this.collection();
}

const ChatDatabaseSchema = DatabaseUniverseGeneratedSchema(
  ...
  converter: DatabaseUniverseObjectConverter<int, ChatDatabase>(
...
int serializeChatDatabase(DatabaseUniverseWriter writer, ChatDatabase object) {
  DatabaseUniverseCore.writeString(writer, 1, object.special_type);
  {
    final list = object.chat_ids;
    final listWriter = DatabaseUniverseCore.beginList(writer, 2, list.length);
    for (var i = 0; i < list.length; i++) {
      DatabaseUniverseCore.writeLong(listWriter, i, list[i]);
    }
    DatabaseUniverseCore.endList(writer, listWriter);
  }
  DatabaseUniverseCore.writeString(writer, 3, object.chat_unique_id);
...
@databaseUniverseProtected
ChatDatabase deserializeChatDatabase(DatabaseUniverseReader reader) {
  ...
  object.special_type = DatabaseUniverseCore.readString(reader, 1) ?? '';
  {
    final length = DatabaseUniverseCore.readList(
        reader, 2, DatabaseUniverseCore.readerPtrPtr);
    {
  ...
  object.chat_unique_id = DatabaseUniverseCore.readString(reader, 3) ?? '';
...
@databaseUniverseProtected
dynamic deserializeChatDatabaseProp(
...
class _ChatDatabaseUpdateImpl implements _ChatDatabaseUpdate {
  ...
  final DatabaseUniverseCollection<int, ChatDatabase> collection;
...
extension ChatDatabaseUpdate on DatabaseUniverseCollection<int, ChatDatabase> {
...
class _ChatDatabaseQueryUpdateImpl implements _ChatDatabaseQueryUpdate {
  ...
  final DatabaseUniverseQuery<ChatDatabase> query;
  final int? limit;
...
extension ChatDatabaseQueryUpdate on DatabaseUniverseQuery<ChatDatabase> {
